package com.classic.core.download.database;

import android.content.ContentValues;
import android.database.Cursor;
import android.database.sqlite.SQLiteDatabase;
import android.util.Log;
import com.classic.core.download.Utils.helper.SqlString;
import com.classic.core.download.core.enums.QueueSort;
import com.classic.core.download.core.enums.TaskStates;
import com.classic.core.download.database.constants.TABLES;
import com.classic.core.download.database.constants.TASKS;
...
public class TasksDataSource {
    ...
    public List<Task> getTasksInState(int state){
        ...
        String query;
        if (state < 6)
        	query = "SELECT * FROM "+TABLES.TASKS+" WHERE "+TASKS.COLUMN_STATE+"="+ SqlString.Int(
              state);
        else
    ...
    public List<Task> getUnCompletedTasks(int priority){
        ...
        String query = "SELECT * FROM " + TABLES.TASKS
                + " WHERE " + TASKS.COLUMN_STATE + "!=" + SqlString.Int(
            TaskStates.END);
        switch (priority){
